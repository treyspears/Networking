...
#define STARTING_SERVER_IP "129.119.246.128"
...
bool UDPClientRecieveMessage( const SOCKET& receiveFromSocket )
{
	...
	do 
	{
		...
		bytesReceived = recvfrom( receiveFromSocket, ( char* )&receivedPacket, BUFFER_LIMIT, 0, (struct sockaddr *)&senderInfo, &senderInfoLength );
...
void OnReceiveReset( const CS6Packet& packet )
{
	...
	g_localPlayer.id.r = packet.playerColorAndID[ 0 ];
	g_localPlayer.id.g = packet.playerColorAndID[ 1 ];
	g_localPlayer.id.b = packet.playerColorAndID[ 2 ];
